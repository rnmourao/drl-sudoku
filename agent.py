...
class Agent():
    ...
    def __init__(self, state_size, action_size, device, seed=2022, filename=""):
    ...
    def step(self, state, action, reward, next_state, done, outcome):
        ...
        if outcome:
            self.statistics[outcome] = self.statistics.setdefault(outcome, 0) + 1
    ...
    def act(self, state, fixed, eps=0.):
        ...
        if random.random() > eps:
            pos = action_values.cpu().data.numpy().reshape((-1, 9))
            flatten = []
            for p in pos:
                m = np.max(p)
                flatten += np.where(p == m, 1, 0).tolist()
            return np.array(flatten)
        else:
            rd = np.random.randint(low=0, high=10, size=(9, 9))
            mask = (fixed != 0)
            rd[mask] = fixed[mask]
            dummies = one_hot_encoder(rd)
            return dummies
